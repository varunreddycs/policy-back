# PolicyPlatform Backend — Project Memory

## Quick Reference

| Task | Command |
|------|---------|
| Start full stack | `docker compose up -d --build` |
| Stop stack | `docker compose down` |
| View logs | `docker compose logs -f api` |
| Run migrations | `uv run alembic upgrade head` |
| Check migration | `uv run alembic current` |
| Run tests | `uv run pytest` |
| Format | `uv run ruff format .` |
| Lint | `uv run ruff check --fix .` |
| Type check | `uv run pyright` |
| Start worker only | `docker compose up -d worker` |

## Port Assignments

| Service | Host Port | Notes |
|---------|-----------|-------|
| API | **8001** | Changed from 8000 — Jarvis agent uses 8000 |
| Postgres | 5433 | pgvector/pgvector:pg16 |
| Azurite Blob | 10000 | devstoreaccount1 |
| Azurite Queue | 10001 | devstoreaccount1 |
| Azurite Table | 10002 | devstoreaccount1 |

Swagger UI: http://localhost:8001/docs

## Stack

- **FastAPI + Uvicorn** — `apps/api/main.py` (`create_app()`)
- **PostgreSQL 16 + pgvector** — primary store + vector similarity (`policy_embeddings`, HNSW cosine, VECTOR(3072))
- **Azure Blob Storage / Azurite** — raw policy uploads (`policy-raw`), extracted artifacts (`policy-extracted`)
- **Azure Storage Queue / Azurite** — background extraction trigger, queue: `policy-extraction`
- **SQLAlchemy 2.x + Alembic** — ORM in `packages/db/models/`, migrations in `migrations/versions/`
- **Pydantic v2** — all API contracts in `packages/core/dtos.py`
- **Azure OpenAI** — embeddings via `packages/embeddings/azure_openai_client.py`
- **structlog** — structured JSON logging throughout

## Migration State

Current head: **007_policy_references**

| # | Migration |
|---|-----------|
| 001 | init — tenants, users, policies, policy_versions, policy_sections, ingest_batches |
| 002 | version_lineage fields |
| 003 | audit_logs table |
| 004 | policy ranking fields (authority_level, department_scope, policy_type) |
| 005 | enable pgvector extension |
| 006 | policy_embeddings table + HNSW index |
| 007 | policy_references |

## Package Layout

```
apps/
  api/
    main.py          # FastAPI app factory; registers all routers
    deps.py          # FastAPI Depends providers
    routers/         # health, ingest, policies, sections, ask, audit, approvals
  worker/
    main.py          # Queue consumer entrypoint
    policy_processor.py
    jobs/embed_backfill.py

packages/
  core/dtos.py       # AskRequest, AnswerResponse, EvidenceCandidate, CitationItem, DecisionInfo
  rag/
    ask_service.py   # Phase 2 orchestration: retrieve → answer → audit
    answer_service.py
  retrieval/
    factory.py       # RETRIEVER_BACKEND switch: pgsql_fts | pgvector | hybrid
    hybrid_provider.py
    pgvector_provider.py
    pgsql_fts_provider.py
    azure_search_provider.py  # STUB — returns []
  ranking/ranker.py  # 3-bucket: dept_match → org_wide → cross_dept
  governance/audit_service.py
  extraction/
    extractor.py     # PDF/DOCX/TXT dispatch + 4000-char chunking
    cleaner.py       # normalize extracted text
  embeddings/azure_openai_client.py
  llm/client.py      # Azure OpenAI Chat Completions; graceful excerpt fallback
  storage/blob_service.py
  db/models/
```

## Retrieval Config (.env)

```
RETRIEVER_BACKEND=hybrid          # pgsql_fts | pgvector | hybrid
EMBEDDINGS_ENABLED=true
EMBEDDINGS_TOP_K=40
FTS_TOP_K=40
AZURE_OPENAI_ENDPOINT=https://mythri-resource.openai.azure.com
AZURE_OPENAI_EMBEDDINGS_DEPLOYMENT=text-embedding-3-large
EMBEDDING_DIM=3072
```

## Phase Completion Status

| Area | Status |
|------|--------|
| Phase 1 — ingestion, versioning, worker, blob/queue, migrations 001–006 | ✅ Done |
| Phase 2 — Ask endpoint, audit, FTS retrieval | ✅ Done |
| Phase 2.5 — pgvector + hybrid retrieval, department-first ranking, embedding backfill | ✅ Done |
| Phase 2.6/2.7 — LLM graceful degradation, refusal detection, retrieval_log in AnswerResponse | ✅ Done |
| Azure Search retriever | ❌ Stub (returns []) |
| Real AuthN/AuthZ | ❌ tenant_id from client, not from auth claims |
| Approvals router | ❌ Empty template |

## Unit Test Baseline

25/25 pass (last verified during Phase 2.7 work):

```
tests/unit/test_answer_service.py        3 passed
tests/unit/test_citation_enforcer.py     2 passed
tests/unit/test_dedupe.py                2 passed
tests/unit/test_embed_backfill.py        2 passed
tests/unit/test_hybrid_retriever.py      5 passed
tests/unit/test_pgvector_retriever.py    1 passed
tests/unit/test_policy_query_service.py  2 passed
tests/unit/test_ranker.py                3 passed
tests/unit/test_retrieval_pgsql_fts.py   3 passed
tests/unit/test_section_cleaner.py       2 passed
```

## Key DTOs (packages/core/dtos.py)

- `AskRequest` — query, tenant_id, department, user_context, top_k
- `AnswerResponse` — answer, citations (list[CitationItem]), decision (DecisionInfo), secondary_evidence, audit_id, retrieval_log
- `EvidenceCandidate` — section_id, policy_id, policy_version_id, score, text, metadata
- `CitationItem` — handle, policy_title, section_title, excerpt, authority_level
- `DecisionInfo` — verdict, confidence, reasoning

## Cosmos DB Migration (separate repo: policy-migrator)

A separate migrator project (`policy-migrator`) was built to migrate data from PostgreSQL to Azure Cosmos DB NoSQL. It lives outside this repo. Connection details in `.Codex/settings.local.json` bash permissions.

## Notes

- Docker port 8001 is the canonical external port for this project (not 8000).
- The `docker compose up -d --build` exit code 1 on Windows is a PowerShell stderr rendering artifact — containers actually start successfully. Verify with `docker compose ps`.
- `.env` has Azurite default shared key hardcoded — this is intentional for local dev.
- `alembic upgrade head` with no output lines after "Will assume transactional DDL" = schema already at head.
- Cosmos DB endpoint and key are in `.Codex/settings.local.json` for the migrator CLI permissions.

## Imported Claude Cowork project instructions

That’s a strong setup. You’re basically building your own local AI engineering team.

The key is: your Master Agent should not code much.
It should plan, decompose, assign, validate, and integrate.

The Child Agents should do the coding.

If you structure it right, this becomes incredibly effective.

Recommended agent hierarchy
1. Master Agent

Role:

understands your business goal
translates it into engineering tasks
decides which child agent should do what
enforces architecture consistency
reviews outputs before merge

It should own:

roadmap
architecture
acceptance criteria
dependency ordering
final integration checks

It should not directly write most implementation code unless needed.

2. Child Agents

You want them specialized.

A. Ingestion Agent

Owns:

crawlers
file normalization
upload/register flow
extraction worker
sectioning logic
B. Retrieval Agent

Owns:

FTS
pgvector
hybrid merge
retrieval scoring
candidate filtering
C. Ranking Agent

Owns:

department-first logic
authority handling
recency rules
conflict prioritization
D. API Agent

Owns:

FastAPI routes
schemas
request/response contracts
audit/replay wiring
E. Frontend Agent

Owns:

React/MUI UI
evidence drawer
citations UX
decision panel
section viewer
F. Infra Agent

Owns:

Docker
Azure deployment
azd
env wiring
storage/postgres/container apps
G. Test Agent

Owns:

integration tests
contract tests
fixture tests
regression guards

This one is very important. It prevents your local agent team from drifting.

Best operating model
Master Agent loop

Your master should run a loop like this:

Step 1 — Interpret request

Example:

“Implement Phase 3 reference extraction for DAS policies”

Step 2 — Break into tasks

Example:

DB migration for policy_references
worker extraction changes
API response updates
frontend section viewer updates
tests
Step 3 — Assign tasks to child agents
Ingestion Agent → extraction logic
API Agent → response schema
Frontend Agent → UI
Test Agent → test cases
Step 4 — Collect outputs

Each child returns:

files changed
rationale
assumptions
test commands
Step 5 — Validate

Master checks:

contract consistency
architecture rules
naming conventions
no duplication
no scope drift
Step 6 — Merge or request revision

Master either:

accepts
sends back correction request
reassigns to another child

That’s the right pattern.

What the Master Agent should always know

You should give the Master Agent a persistent “system memory” document containing:

A. Product identity
what the platform is
what it is not
target users
business rules
B. Architecture
ingestion flow
hybrid retrieval design
department-first ranking
explainability layer
roadmap
C. Coding standards
monorepo structure
naming conventions
logging rules
env variable policy
test requirements
D. Current state
completed phases
open work
known issues
future roadmap

Without this, your agents will drift badly.

What every Child Agent should receive

Every child prompt should contain 5 things:

1. Goal

What to build.

2. Scope boundaries

What NOT to touch.

3. Relevant architecture rules

Example:

hybrid retrieval is permanent
department-first override must remain
no breaking API changes
4. File targets

Exactly which files/folders the agent may modify.

5. Acceptance criteria

How success is judged.

This keeps them disciplined.

Important rule: file ownership

Give each child agent a clear ownership map.

Example:

Retrieval Agent can edit:
packages/retrieval/*
retrieval tests
Frontend Agent can edit:
apps/web/*
API Agent can edit:
apps/api/*
schemas/routes/services

This avoids random edits across the repo.

Recommended Master Agent output format

Your Master Agent should answer in a structured way like this:

Goal
Context
Task Breakdown
Child Agent Assignments
Dependencies
Acceptance Criteria
Validation Checklist
Merge Order

This makes it much easier to manage.

Example of a real workflow

You say:

“Implement Phase 3 reference extraction”

Master Agent should produce:

Goal

Capture external legal/policy references from extracted policy sections and expose them in API/UI.

Child assignments
Ingestion Agent:
extract links from section text / HTML
persist policy_references
API Agent:
add references to section detail response
Frontend Agent:
show references in section viewer
Test Agent:
add unit/integration tests
Constraints
do not change /v1/ask response shape except additive fields
do not query references yet
references are view-only in this phase
Acceptance criteria
links stored
visible in UI
tests pass

That’s the kind of orchestration you want.

Critical design principle

Your Master Agent should think like:

architect + tech lead + reviewer

Not like:

coder

Your Child Agents should think like:

specialist implementers

That separation is what makes the system powerful.

Suggested local folder for agent instructions

You may want a structure like:

/agents
  /master
    system_prompt.md
    architecture_memory.md
    roadmap.md
  /children
    ingestion_agent.md
    retrieval_agent.md
    ranking_agent.md
    api_agent.md
    frontend_agent.md
    infra_agent.md
    test_agent.md

This makes your desktop assistants consistent.

Strong recommendation

Before you start coding with agents, create these 3 master docs first:

1. Architecture Memory

Contains:

all phases
hybrid design
ranking rules
repo structure
2. Coding Constitution

Contains:

no breaking changes without approval
additive API changes only
tests required
logging required
env-driven config only
3. Agent Role Definitions

Contains:

each child’s responsibilities
editable paths
forbidden areas
output format

These three docs will massively improve quality.

Biggest risk with your approach

The danger is not that agents fail to code.

The danger is:

duplicated logic
inconsistent schemas
breaking architecture rules
scattered changes across repo

The Master Agent exists to prevent that.

Best next move

You should create:

Master Agent system prompt
Child Agent role prompts
Task handoff template

That will make your desktop agent team usable immediately.

If you want, I’ll write all three next:

Master Agent prompt
Child Agent prompt pack
Standard task delegation template
