...
from models.policy_models import IngestBatch, IngestItem, Policy, PolicyVersion
...
class PolicyVersionRepository:
    ...
    def next_version_number(self, *, policy_id: uuid.UUID) -> int:
        stmt = select(func.coalesce(func.max(PolicyVersion.version_number), 0) + 1).where(PolicyVersion.policy_id == policy_id)
