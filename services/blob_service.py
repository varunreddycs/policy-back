...
def _storage_api_version() -> Optional[str]:
	# Needed for Azurite compatibility when the SDK defaults to a newer service version.
...
class BlobServiceConfig:
	...
	def from_env() -> "BlobServiceConfig":
		...
		if not account_name:
			# Try to parse from URL: https://{name}.blob.core.windows.net
			try:
...
class BlobService:
	...
	Auth options:
	- Shared key via env var AZURE_STORAGE_ACCOUNT_KEY
	- Entra ID (Managed Identity / workload identity / dev login) via DefaultAzureCredential
	  when no account key is provided
	"""
	...
	def get_blob_uri(self, container_name: str, blob_path: str) -> str:
		"""Returns the canonical URI for a blob (without SAS).

		Important for local Docker dev:
		- The service client may use an internal endpoint (e.g., http://azurite:10000)
		- But the returned URI should be host-accessible (e.g., http://127.0.0.1:10000)
		"""
		if not container_name or not container_name.strip():
			raise ValueError("container_name must be non-empty")
		blob_path = _normalize_blob_path(blob_path)
	...
	def generate_upload_sas_url(
		...
		*,
		...
		"""Generates a SAS URL for uploading (create/write) a blob.

		This does not create the blob; it returns a time-limited URL suitable for a client upload.
		"""
		...
		try:
			...
			# User delegation SAS using Entra ID
			service_client = self._get_service_client()
			udk = service_client.get_user_delegation_key(key_start_time=starts_on, key_expiry_time=expires_on)
			sas_token = generate_blob_sas(
				account_name=self._config.account_name,
				container_name=container_name,
				blob_name=blob_path,
	...
	def download_blob_bytes(
		...
		*,
		...
		"""Downloads a blob into memory as bytes."""
	...
	def upload_blob_bytes(
		...
		*,
		...
		"""Uploads bytes to a blob and returns the blob URI."""
		blob_client = self._get_blob_client(container_name, blob_path)
		try:
