...
from repositories.ingestion_repositories import (
...
class IngestionService:
    ...
    - Compute content + metadata hashes
    - Detect duplicates
    - Create policy + immutable policy version
    - Set parse_status='pending'
    - Publish a queue message for async extraction

    This service does not perform extraction.
    """
        ...
        *,
        ...
        with self._session.begin():
            ...
            try:
                ...
                # Covers uniqueness constraints and concurrent version creation.
            ...
            # Link ingest item to the resulting version.
        ...
        # Enqueue after commit so the worker sees committed records.
        try:
            ...
            # Best-effort status update (does not roll back version creation).
            with self._session.begin():
        ...
        # Mark ingest item as queued.
        with self._session.begin():
            self._items.set_status(item_id=dto.ingest_item_id, status="queued")

            # Best-effort batch status update for demo visibility.
            batch = self._batches.get(batch_id)
