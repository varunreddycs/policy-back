...
class JsonLogFormatter(logging.Formatter):
	...
	def format(self, record: logging.LogRecord) -> str:
		...
		# Capture common code location fields.
		payload["code"] = {
		...
		# Include extras (anything non-standard added via logger.*(extra={...})).
		for key, value in record.__dict__.items():
			...
			if key in {
				...
				continue
			# Avoid clobbering top-level keys.
			if key in payload:
				continue
			payload[key] = value
...
def configure_logging(
	*,
	...
	- Emits JSON lines to stdout (Azure App Service friendly)
	- Adds correlation_id + request_id via RequestContextFilter
	"""
	...
	# Ensure common servers propagate into root.
